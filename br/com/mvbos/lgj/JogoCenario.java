...
public class JogoCenario extends CenarioPadrao {
	...
	public void rankJogadores() {
		...
		JOptionPane.showMessageDialog(null,
				posicao_Ultimo_Jogador + " no Ranking / Nome: " + J.getNome() + " / Pontos: " + J.getPontuacao());

		JOptionPane.showMessageDialog(null, "Nome: " + J.getNome() + " " + "Pontos: " + J.getPontuacao(), "Ranking",
	...
	public void desenhar(Graphics2D g) {
		...
		for (int col = 0; col < grade.length; col++) {
			for (int lin = 0; lin < grade[0].length; lin++) {
				...
				int x = ajuste_de_tela + col * largBloco + ESPACAMENTO;
		...
		if (peca != null) {
			...
			for (int col = 0; col < peca.length; col++) {
				for (int lin = 0; lin < peca[col].length; lin++) {
					if (peca[lin][col] != 0) {
						int x = ajuste_de_tela + (col + ppx) * largBloco + ESPACAMENTO;
		...
		for (int col = 0; col < prxPeca.length; col++) {
			for (int lin = 0; lin < prxPeca[col].length; lin++) {
				...
				int x = 757 + col * miniatura + ESPACAMENTO;
				int y = 70 + lin * miniatura + ESPACAMENTO;
		...
		for (int col = 0; col < prxPeca2.length; col++) {
			for (int lin = 0; lin < prxPeca2[col].length; lin++) {
				...
				int x = 842 + col * miniatura + ESPACAMENTO;
				int y = 70 + lin * miniatura + ESPACAMENTO;
		...
		for (int col = 0; col < prxPeca3.length; col++) {
			for (int lin = 0; lin < prxPeca3[col].length; lin++) {
				...
				int x = 923 + col * miniatura + ESPACAMENTO;
				int y = 70 + lin * miniatura + ESPACAMENTO;
		...
		texto.desenha(g, "Level:" + nivel, 20, 35);
		texto.desenha(g, "Linhas Eliminadas:" + contador_feitas, 20, 60);
		texto.desenha(g, "Pontos:" + pontos, 20, 85);
		texto.desenha(g, "Status: ", 20, 142);
		texto.desenha(g, "ESTRUTURA SEGUINTE", 775, 35);

		g.drawLine(250, 0, 250, 672);
		g.drawLine(0, 108, 250, 108);
		g.drawLine(0, 160, 250, 160);

		texto.setCor(Color.WHITE);
		texto.desenha(g, ": " + cont_L_esquerda, 100, 215);
		texto.desenha(g, ": " + cont_L_direita, 100, 285);
		texto.desenha(g, ": " + cont_T, 100, 345);
		texto.desenha(g, ": " + cont_Raio_esquerda, 100, 410);
		texto.desenha(g, ": " + cont_Raio_direita, 100, 480);
		texto.desenha(g, ": " + cont_quadrado, 100, 542);
		texto.desenha(g, ": " + cont_reta, 100, 610);

		g.drawLine(750, 0, 750, 672);
		g.drawLine(750, 50, 1000, 50);
		g.drawLine(750, 175, 1000, 175);
		g.drawLine(834, 50, 834, 175);
		g.drawLine(918, 50, 918, 175);
		texto.desenha(g, "Tetris by Braule", 812, 650);

		int miniatura_fixada = largBloco / 3;
		int[][] prxPeca_fixada0 = Peca.PECAS[0];
		...
		for (int col = 0; col < prxPeca_fixada0.length; col++) {
			for (int lin = 0; lin < prxPeca_fixada0[col].length; lin++) {
				...
				int x = 40 + col * miniatura_fixada + ESPACAMENTO;
				int y = 185 + lin * miniatura_fixada + ESPACAMENTO;
		...
		int[][] prxPeca_fixada1 = Peca.PECAS[1];
		...
		for (int col = 0; col < prxPeca_fixada1.length; col++) {
			for (int lin = 0; lin < prxPeca_fixada1[col].length; lin++) {
				...
				int x = 40 + col * miniatura_fixada + ESPACAMENTO;
				int y = 255 + lin * miniatura_fixada + ESPACAMENTO;
		...
		int[][] prxPeca_fixada2 = Peca.PECAS[2];
		...
		for (int col = 0; col < prxPeca_fixada2.length; col++) {
			for (int lin = 0; lin < prxPeca_fixada2[col].length; lin++) {
				...
				int x = 40 + col * miniatura_fixada + ESPACAMENTO;
				int y = 325 + lin * miniatura_fixada + ESPACAMENTO;
		...
		int[][] prxPeca_fixada3 = Peca.PECAS[3];
		...
		for (int col = 0; col < prxPeca_fixada3.length; col++) {
			for (int lin = 0; lin < prxPeca_fixada3[col].length; lin++) {
				...
				int x = 40 + col * miniatura_fixada + ESPACAMENTO;
				int y = 380 + lin * miniatura_fixada + ESPACAMENTO;
		...
		int[][] prxPeca_fixada4 = Peca.PECAS[4];
		...
		for (int col = 0; col < prxPeca_fixada4.length; col++) {
			for (int lin = 0; lin < prxPeca_fixada4[col].length; lin++) {
				...
				int x = 40 + col * miniatura_fixada + ESPACAMENTO;
				int y = 450 + lin * miniatura_fixada + ESPACAMENTO;
		...
		int[][] prxPeca_fixada5 = Peca.PECAS[5];
		...
		for (int col = 0; col < prxPeca_fixada5.length; col++) {
			for (int lin = 0; lin < prxPeca_fixada5[col].length; lin++) {
				...
				int x = 40 + col * miniatura_fixada + ESPACAMENTO;
				int y = 520 + lin * miniatura_fixada + ESPACAMENTO;
		...
		int[][] prxPeca_fixada6 = Peca.PECAS[6];
		...
		for (int col = 0; col < prxPeca_fixada6.length; col++) {
			for (int lin = 0; lin < prxPeca_fixada6[col].length; lin++) {
				...
				int x = 40 + col * miniatura_fixada + ESPACAMENTO;
				int y = 575 + lin * miniatura_fixada + ESPACAMENTO;
		...
		if (estado == Estado.JOGANDO && Jogo.pausado == false) {
			texto.desenha(g, "Jogando! ", 87, 142);
		}
		...
		if (estado != Estado.JOGANDO) {
			...
			if (estado == Estado.GANHOU)
				...
				texto.desenha(g, "Deu Ruim! ", 87, 142);
		}
	}
}